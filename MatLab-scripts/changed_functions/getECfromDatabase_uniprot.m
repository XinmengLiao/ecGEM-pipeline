...
data    = loadDatabases('uniprot', modelAdapter);
...
for i = 1:n
    ...
    if ~isempty(gns)
        ...
        %if (isempty(new_EC) || endsWith(new_EC,'-'))
            %Find match in KEGG
...
if strcmpi(action,'display') && ~isempty(conflicts{1})
    displayErrorMessage(conflicts,uniprot)
end
...
function displayErrorMessage(conflicts,uniprot)
