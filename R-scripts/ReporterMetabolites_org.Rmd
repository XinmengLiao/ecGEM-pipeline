...
rma <- function(network,pvalue,microbeFoldChanges = F,filter.extreme = F,reaction){
  ...
  if(filter.extreme == T){
  # filter pvalue = 1
    network <- network %>% filter(zscore!= -15) 
  }
  ...
  if(microbeFoldChanges == T){
    network.up <- network %>% filter(logFC > 0)
    network.down <- network %>% filter(logFC < 0) 
    met.up <- rma.sub(network.up)
    met.down <- rma.sub(network.down)
    return(list(all.met = metabolite_scores, increased.met = met.up, decreased.met = met.down))
  }else{
    return(list(all.met = metabolite_scores))
  }
}

rma.all <- rma(network.all,pvalue = "PValue",reaction = fva.all1,
               filter.extreme = T,microbeFoldChanges = T)
rma.uptake <- rma(network.uptake,pvalue = "PValue",reaction = fva.all1,
                  filter.extreme = T,microbeFoldChanges = T)
