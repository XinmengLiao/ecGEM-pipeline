...
rma <- function(network,pvalue,microbeFoldChanges = F,reaction){
  ...
  for (i in 1:n_mets){
    ...
    if(microbeFoldChanges == T){
      met.network$zscore_direction = NA
      for (rows in 1:nrow(met.network)){
        if(met.network$direction[rows] =="Uptake"){
          met.network$zscore_direction[rows] <- -1 *  met.network$zscore_regulate[rows]
        }else if (met.network$direction[rows] =="Secrete"){
          met.network$zscore_direction[rows] <- 1 * met.network$zscore_regulate[rows]
        }
      }
  ...
  metabolite_scores <- tibble(metabolite = metNGenes,
                                  k = n_microbe,
                                  Z_raw = metZScores, ### change here
                                  gene_Zs = microbe.zscore.list)
  
  background_stats <- metabolite_scores %>%
  rowwise() %>%
  mutate(
    background = list(replicate(n_sim, {
      sampled_zs <- sample(all_zscores, k, replace = TRUE)
      
      # randomly assign the directions for uptake/secrete
    ...
    # two-tailed test
    p_corrected = (2 * (1 - pnorm(abs(Z_corrected))))
  ) %>%
  ungroup() %>% 
  ...
  if(microbeFoldChanges == T){
    met.up <- top_mets %>% filter( Z_raw > 0 ) 
    met.down <- top_mets %>% filter(Z_raw < 0 ) 
    return(list(all.top.met = top_mets, up.met = met.up, down.met = met.down))
  }else{
